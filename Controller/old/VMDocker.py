...
class VMDocker(object):
    ...
    def set_run_app(self, app_name):
        try:
            f = open(WORK_PATH + '\cmd\\save_redis_key.conf', 'w')
            f.write(app_name)
            f.close()
        ...
        guest_path = '"' + WORK_PATH + '\cmd\\save_redis_key.conf"'
        host_path = '"C:\workspace\pyWorks\AppSimulator\cmd\\save_redis_key.conf"'
        time.sleep(1)
